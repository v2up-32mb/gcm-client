...
- DoH：`dohURL` 非空则单服务器，否则使用内置备用 DoH 列表（如 `doh.090227.xyz/CMLiussss`、`doh.pub`）
- DNS 预热默认关闭（`EnableDNSWarmup=false`），避免 Android 冷启动冲突；后续可通过参数打开
...
| `EchDns` | ECH 使用的 DoH 服务器（如 `dns.alidns.com/dns-query`） | `dohURL` | `cfg.DoHUrl`，空则内置备用列表 |
...
func StartSocksProxy(
    ...
    enableECH, disableIPv6Route, verbose bool,
) error
...
- `dohURL`：DoH 服务器（空则使用 GCM 内置备用 DoH 列表）
